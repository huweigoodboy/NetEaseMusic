...
import com.huwei.neteasemusic.inter.OnLrcSearchClickListener;
...
//		    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (canDrawLine) {
            ...
            canvas.drawText(TimeUtil.getDuration(lrcLists.get(pos).getLrcTime()), 42, scrollY + height / 2 - 2, linePaint);
        }
    }
    ...
    private void init() {
        ...
        norTextSize = DensityUtil.sp2px(mContext, 16);
        lightTextSize = DensityUtil.sp2px(mContext, 18);
        tipsTextSize = DensityUtil.sp2px(mContext, 20);
        textHeight = norTextSize+DensityUtil.dip2px(mContext,10);
        ...
        //设置画笔颜色
        currentPaint.setColor(getResources().getColor(R.color.colorPrimary));
    ...
    boolean handleTouchLrcOK(int action) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if(pos!=-1)
                    MusicManager.get().seekTo(lrcLists.get(pos).getLrcTime());
                canDrawLine = false;
                pos =-1;
                this.invalidate();
                break;
